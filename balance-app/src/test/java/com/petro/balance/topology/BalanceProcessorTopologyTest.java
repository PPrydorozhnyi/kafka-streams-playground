...
class BalanceProcessorTopologyTest {
    ...
    void balanceTopologyTest() {
        ...
        try (TopologyTestDriver topologyTestDriver = new TopologyTestDriver(topology, streamsConfiguration)) {
            TestInputTopic<String, BalanceChangeEvent> inputTopic = topologyTestDriver.createInputTopic(
                    IN_TOPIC, new StringSerializer(), balanceChangeSerde.serializer());

            TestOutputTopic<String, BalanceStateEvent> outputTopic = topologyTestDriver.createOutputTopic(
                    OUT_TOPIC, new StringDeserializer(), balanceStateSerde.deserializer());
            ...
            final var thirdEvent = new BalanceChangeEvent(
                    name, 40, Instant.ofEpochMilli(System.currentTimeMillis()).plusSeconds(30));
